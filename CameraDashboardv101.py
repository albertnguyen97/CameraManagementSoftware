...
class AddRTSPDialog(QDialog):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Add RTSP Link")
        ...
        # Nhập link RTSP
        self.rtsp_input = QtWidgets.QLineEdit(self)
        self.rtsp_input.setPlaceholderText("Enter RTSP Link")
        layout.addWidget(self.rtsp_input)

        # Nút xác nhận
        self.add_button = QtWidgets.QPushButton("Add", self)
        self.add_button.clicked.connect(self.accept)  # Đóng dialog khi nhấn nút
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.widgetDevice.setObjectName("widgetDevice")
        self.tabWidget.addTab(self.widgetDevice, "Device")
        self.device_layout = QVBoxLayout(self.widgetDevice)

        # Nút Add
        self.add_button = QPushButton("Add")
        self.add_button.clicked.connect(self.open_add_dialog)
        self.device_layout.addWidget(self.add_button,
                                     alignment=Qt.AlignmentFlag.AlignRight)  # Đặt nút ở góc trên bên phải

        # Bảng để hiển thị các RTSP link
        self.rtsp_table = QTableWidget(self.widgetDevice)
        self.rtsp_table.setColumnCount(1)
        self.rtsp_table.setHorizontalHeaderLabels(["RTSP Links"])
        self.device_layout.addWidget(self.rtsp_table)
        # User Page
    ...
    def open_add_dialog(self):
        dialog = AddRTSPDialog()
        if dialog.exec() == QtWidgets.QDialog.DialogCode.Accepted:
            rtsp_link = dialog.get_rtsp_link()
            self.add_rtsp_to_table(rtsp_link)

    def add_rtsp_to_table(self, rtsp_link):
        row_position = self.rtsp_table.rowCount()
        self.rtsp_table.insertRow(row_position)  # Thêm hàng mới
        self.rtsp_table.setItem(row_position, 0, QtWidgets.QTableWidgetItem(rtsp_link))  # Điền liên kết vào bảng
